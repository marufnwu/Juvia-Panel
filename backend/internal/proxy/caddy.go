...
// Proxies all non-API traffic to the Next.js server.
func (c *Caddy) writePanelUIBlock(builder *strings.Builder) {
	port := c.panelUIPort
	builder.WriteString(fmt.Sprintf(":%d {\n", port))

	builder.WriteString("    # WebSocket — must come before generic /api handler\n")
	...
	builder.WriteString("    # API routes — proxy to Go backend\n")
	...
	builder.WriteString("    # Everything else — proxy to Next.js server\n")
	builder.WriteString("    # Next.js handles all UI routes, middleware, and static file serving\n")
