...
builder.WriteString(fmt.Sprintf(":%d {\n", port))
	...
	builder.WriteString("    # Next.js static files (_next/static, etc.)\n")
	...
	builder.WriteString("            Content-Security-Policy \"default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' blob:; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; font-src 'self' data:; connect-src 'self' ws: wss:; worker-src 'self' blob:; frame-ancestors 'none'; base-uri 'self'; form-action 'self'\"\n")
	builder.WriteString("        }\n")
	...
	builder.WriteString("    # Static assets\n")
	...
	builder.WriteString("    # WebSocket endpoint - explicit handling for upgrades\n")
	...
	builder.WriteString("    # Proxy API requests to Go backend\n")
	...
	builder.WriteString("    # Rewrite rules for Next.js static export catch-all routes\n")
	builder.WriteString("    @appsRoute path /apps/*\n")
	builder.WriteString("    handle @appsRoute {\n")
	builder.WriteString("        rewrite /apps/* /apps/_/index.html\n")
	builder.WriteString("        file_server {\n")
	builder.WriteString("            root /opt/panel/ui/out\n")
	builder.WriteString("        }\n")
	builder.WriteString("    }\n\n")
	builder.WriteString("    @servicesRoute path /services/*\n")
	builder.WriteString("    handle @servicesRoute {\n")
	builder.WriteString("        rewrite /services/* /services/_/index.html\n")
	builder.WriteString("        file_server {\n")
	builder.WriteString("            root /opt/panel/ui/out\n")
	builder.WriteString("        }\n")
	builder.WriteString("    }\n\n")
	builder.WriteString("    @cronRoute path /cron/*\n")
	builder.WriteString("    handle @cronRoute {\n")
	builder.WriteString("        rewrite /cron/* /cron/_/index.html\n")
	builder.WriteString("        file_server {\n")
	builder.WriteString("            root /opt/panel/ui/out\n")
	builder.WriteString("        }\n")
	builder.WriteString("    }\n\n")
